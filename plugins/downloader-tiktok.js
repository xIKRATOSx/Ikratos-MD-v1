...
var handler = async (m, { conn, args }) => {
  if (!args[0]) {
    throw '*Give TikTok Url...*';
  }

  try {
    await conn.reply(m.chat, 'Wait, It may take 2-5 min depending on size of video\nThe video is being downloaded...', m);
    ...
    await conn.reply(m.chat, `Here\'s the video\n\n${fig}`, m);
  } catch (error) {
