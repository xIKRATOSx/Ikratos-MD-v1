...
handler.before = async function (m) {
    ...
    if (!(id in this.siapakahaku)) return conn.sendButton(m.chat, 'The matter is over', author, ['who am i', '/siapakahaku'], m)
    if (m.quoted.id == this.siapakahaku[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*True!*\n+${this.siapakahaku[id][2]} XP`, author, ['who am i', '/siapakahaku'], m)
