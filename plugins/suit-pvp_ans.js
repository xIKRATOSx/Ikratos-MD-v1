  ...
  if (room) {
    ...
    if (m.sender == room.p2 && /^(acc(ept)?|terima|gas|oke?|tolak|gamau|nanti|ga(k.)?bisa)/i.test(m.text) && m.isGroup && room.status == 'wait') {
      if (/^(tolak|gamau|nanti|ga(k.)?bisa)/i.test(m.text)) {
        this.reply(m.chat, `@${room.p2.split`@`[0]} reject the suit, the suit is cancelled`, m)
      ...
      m.reply(`The suit has been sent to chat
@${room.p.split`@`[0]} dan 
@${room.p2.split`@`[0]}
...
click wa.me/${conn.user.jid.split`@`[0]}`, m.chat, 
      ...
      if (!room.pilih) this.sendButton(room.p, `Please select Win +${room.poin}XP\nLose -${room.poin_lose}XP`, wm, null, [[ 'stone🗿', 'Batu'], ['Paper📄', 'Kertas'], ['Scissors✂️', 'Gunting']], m)
      if (!room.pilih2) this.sendButton(room.p2, `Please select Win +${room.poin}XP\nLose -${room.poin_lose}XP`, wm, null, [['stone🗿', 'Batu'], ['Paper📄', 'Kertas'], ['Scissors✂️', 'Gunting']], m)
      room.waktu_milih = setTimeout(() => {
        if (!room.pilih && !room.pilih2) this.reply(m.chat, `Both players do not intend to play,\nSuit is cancelled`)
        else if (!room.pilih || !room.pilih2) {
          win = !room.pilih ? room.p2 : room.p
          this.reply(m.chat, `@${(room.pilih ? room.p2 : room.p).split`@`[0]} don\'t choose a suit, game over`, m)
